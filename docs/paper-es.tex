...
\title{Frankestein Transformer:\\Biblioteca Unificada Codificador-Decodificador, CLI y Notas de Diseño Basadas en Investigación}
\author{
Erick F. Merino M. \\
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Entrenamiento Agnóstico a la Arquitectura}: Infraestructura de entrenamiento común que admite líneas base de atención densa (estándar, sigmoide), alternativas recurrentes (RetNet, Mamba, bloques estilo ODE), enrutamiento de profundidad adaptativa (Mixture-of-Depths), bloques de memoria condicional (Engram), patrones de atención dispersa (Sparse Transformer, Longformer, BigBird, SparseK, NSA, SpargeAttn, FASA) y mecanismos con compuerta (GLA, DeltaNet, Gated DeltaNet, HGRN2, FoX, Gated Softmax).
    \item \textbf{Marco de Enrutamiento de Optimizadores}: Grupos de hiperparámetros con prefijo que permiten control granular sobre incrustaciones, capas de normalización, bloques recurrentes, bloques de atención y otros subconjuntos de parámetros a través de optimizadores diversos, incluyendo la familia APOLLO (Apollo, Apollo-Mini, Q-Apollo).
    ...
    \item \textbf{Herramientas de Confiabilidad}: Validación automatizada con suites de pruebas unitarias, verificación de presets YAML y automatización de CI en múltiples versiones de Python.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Canal de Validación Automatizada}: Integración continua y pruebas unitarias ampliadas para verificar rutas de entrenamiento, integración de optimizador/esquema y compatibilidad de ejemplos YAML.
\end{enumerate}
